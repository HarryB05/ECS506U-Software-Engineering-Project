-- Rollback admin role support: restore pre-admin policies and RPC checks.

-- 1) Remove helper
drop function if exists public.is_admin();

-- 2) Restore RLS policies to their previous definitions

-- pet_profiles (restore)
...
-- bookings / booking_requests / junction tables (restore)
...
-- 3) Restore SECURITY DEFINER RPC checks (no admin bypass)
